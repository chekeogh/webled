...
$(function() {
	'use strict';
var	colorJson={"r":59,"g":118,"b":191};
...
$('#update').on('click', updateServerAddress);
		...
		function updateColorInput() {
			...
			switch ($formatInput.val()) {
				...
				case 'RGB':
					...
					colorJson='{"r":'+ RGB.red +',"g":'+ RGB.green + ',"b":'+ RGB.blue + '}';
					break;
				case 'Hex':
...
//---------------------- Initialize this color picker
	...
	console.log("Send off command..");

		$.ajax({
			type: 'GET',
			url: server + '/strip/status/off/',
			success: function(data){
			// Do some nice stuff here
			console.log("Sent");
			},
	...
	console.log("Send on command..");

		$.ajax({
			type: 'GET',
			url: server + '/strip/status/on/',
			success: function(data){
			// Do some nice stuff here
			console.log("Sent");
			},
